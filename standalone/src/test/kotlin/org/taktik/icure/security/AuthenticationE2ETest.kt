...
class AuthenticationE2ETest(
    ...
    @Autowired val httpClient: TestHttpClient,
    @Autowired val bridgeConfig: BridgeConfig
) : StringSpec({

    val khmerUrl = "http://127.0.0.1:$port"
    ...
    "Can use the JWT issued by the Kraken" {
        ...
        val currentUserRaw = httpClient.makeGetRequest("$khmerUrl/rest/test/auth/currentuser", mapOf("Authorization" to "Bearer $token"))
    ...
    "A request with an invalid JWT results in a 401" {
        httpClient.makeGetRequest("$khmerUrl/rest/test/auth/currentuser", mapOf("Authorization" to "Bearer not.a.valid-token"), expectedResponse = 401)
    }

    "Can access the endpoints as an Admin" {
        val adminUser = createAdminUser(bridgeConfig.iCureUrl, KmehrTestApplication.masterHcp.login, KmehrTestApplication.masterHcp.password)
