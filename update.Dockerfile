...
RUN git clone https://github.com/icure/kraken-lite.git --depth=1 --recurse-submodules && \
    ...
    echo "$gpgKey" | base64 -d | gpg --batch --import && \
    GPG_KEY_ID=$(gpg --list-secret-keys --keyid-format LONG dev@icure.com | grep sec | awk '{print $2}' | cut -d'/' -f2) && \
    ...
    git add . && \
    git commit -S -m "Bumped kraken-common and kmehr module" && \
